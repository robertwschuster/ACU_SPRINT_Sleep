...
rtf2df <- function(file) {
  ...
  ug <- grep('\u00d9',df)
  # if kiloohm ('\u2126) was replaced with U-grave ('\u00d9') assume that en dash ('\u2013) was not split correctly,
...
sleepMetrics <- function(data) {
  ...
  # start recording
  sr <- somfind(data, 'Start Recording', function(x) as.POSIXct(x, format = '%H:%M:%S'))
...
srtemp <- "C:/Users/s4548745/Desktop/ACU/ACU_SPRINT_Sleep/Report templates/Report template_somfit_CG.xlsx"
...
for (s in subjects) {
  ...
  # Save the workbook
  saveWorkbook(wb, paste0(fp,'/',s,'-somfit_report_CG.xlsx'))
}
