...
sleepMetrics_MP <- function(data) {
  ...
  # Total data received - value (%) and rating
  ...
  out <- data.frame(df, check.names = FALSE)
}
...
sleepMetrics_CG <- function(data) {
  ...
  # sleep latency
  df$`Sleep latency (min)` <- somfind(data, "Sleep Latency", function(x) as.numeric(x) %>% `/`(60))
  df$`Sleep latency to 10 min (min)` <- somfind(data, " Latency (to 10 min sleep)", function(x) as.numeric(x) %>% `/`(60))
  df$`REM latency (min)` <- somfind(data, " REM latency", function(x) as.numeric(x) %>% `/`(60))
  # WASO
  df$`WASO (min)` <- somfind(data, "Wake after sleep onset (WASO)", function(x) as.numeric(x) %>% `/`(60))
  # sleep efficiency
  df$`Sleep efficiency (%)` <- somfind(data, "Sleep efficiency", as.numeric)
  ...
  df$`Awakenings (total)` <- somfind(data, "Number of awakenings", as.numeric)
  # pAHI
  df$`Resp events` <- somfind(data, "Qty Resp Events Total", as.numeric)
  df$pAHI <- somfind(data, "pAHI Total", as.numeric)
  # SpO2
  df$`Mean SpO2 (sleep)` <- somfind(data, "Average SpO2 (Sleep)", as.numeric)
  df$`Min SpO2 (sleep)` <- somfind(data, "Lowest SpO2 (Sleep)", as.numeric)
  df$`Mean desat (sleep)` <- somfind(data, "Average Desat (sleep)", as.numeric)
  df$`Mean desat with resp events` <- somfind(data, "Average desaturation with respiratory events", as.numeric)
  # HRV and pulse
  df$HRV <- somfind(data,"HRV (Average)", as.numeric)
  df$`Pulse (mean)` <- somfind(data, "Pulse Rate (Average)", as.numeric)
  df$`Pulse (min)` <- somfind(data, "Pulse Rate (Lowest)", as.numeric)
...
subNames <- function(filepath, filename) {
  ...
  for (i in 1:length(filename)) {
    ...
    # Participant ID
    subjects[i, 2] <- temp$`Metric Value`[which(temp$`Metric Name` == "URN")]
  }
  return(subjects)
}
...
importFiles <- function(filepath, filename, mpcg) {
  ...
  for (i in 1:length(filename)) {
    ...
    temp <- rtf2df(filepath[i])
    # Participant ID
    subject <- temp$`Metric Value`[which(temp$`Metric Name` == "URN")]
...
sleepData <- function(data, rsn, nse) {
  ...
  dex[3, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`Total recording time (min)`)
  dex[4, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`Sleep latency (min)`)
  dex[5, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`WASO (min)`)
  dex[6, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`Total sleep time (min)`)
  dex[7, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE), 1), x = data$sfMetrics$`Sleep efficiency (%)`)
  
  awake <- data$sfMetrics$`Time available for sleep (min)` - data$sfMetrics$`Total sleep time (min)`
  dex[8, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`N1/N2 (min)`)
  dex[9, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`N3 (min)`)
  dex[10, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = data$sfMetrics$`REM (min)`)
  dex[11, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = awake)
  awake <- awake / data$sfMetrics$`Total sleep time (min)` * 100
  dex[12, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE)), x = data$sfMetrics$`N1/N2 (%)`)
  dex[13, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE)), x = data$sfMetrics$`N3 (%)`)
  dex[14, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE)), x = data$sfMetrics$`REM (%)`)
  dex[15, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE)), x = awake)
  ...
  slf$`Bed time avg` <- mean(slf$`Bed time`, na.rm = TRUE)
  slf$`Wake-up time avg` <- mean(slf$`Wake-up time`, na.rm = TRUE)
  ...
  slf <- data.frame(slf, check.names = FALSE)
  slf[slf == 0] <- NA
...
sleepReport <- function(data, srtemp) {
  ...
  # Add summary table
  addDataFrame(data$dex, sheets$Summary, startRow = 2, startColumn = 2, col.names = FALSE, row.names = FALSE)
  
  # Add data for figures
  addDataFrame(data$slf, sheets$Figures, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, showNA = TRUE)
  
  # Add data for analysis
  addDataFrame(data$sld, sheets$Data, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, showNA = TRUE)
...
durationPlot <- function(slf,rsn) {
  ...
  ggplot(df) +
    geom_col(aes(x = Date, y = Hours, fill = Phase), position = position_stack(reverse = TRUE), width = 1) + 
...
patternPlot <- function(slf,sld) {
  ...
  ggplot(df) +
    geom_col(aes(x = Date, y = Time, fill = Night, alpha = Alpha), position = position_stack(reverse = TRUE), width = 1) +
