...
cyc <- function(df, P2, P3) {
  ...
  df <- cbind(df[, 1:2, drop = FALSE], replicate(2, df[, 2]),
              df[, 3, drop = FALSE], replicate(2, df[, 3]),
              df[, 4, drop = FALSE], replicate(2, df[, 4]),
              df[, 5, drop = FALSE], replicate(2, df[, 5]),
              df[, 6, drop = FALSE], replicate(2, df[, 6]),
              df[, 7, drop = FALSE],
              df[, 8, drop = FALSE], replicate(2, df[, 8]),
              df[, 9:11, drop = FALSE])
...
timeInBed <- function(df, CS, CE) {
  ...
  tib[1, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = df$Duration[rest])
  # Time to fall asleep (h:min)
  if ("Onset Latency" %in% colnames(df)) {
    tib[2, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = df$`Onset Latency`[sleep])
  }
  # Sleep per night (h:min)
  tib[3, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = df$`Sleep Time`[sleep])
  # Time awake / light sleep per night (h:min)
  tib[4, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = df$`Wake Time`[sleep])
  # Sleep efficiency (quality - %)
  tib[5, 1:3] <- sapply(funs, function(fun, x) round(fun(x, na.rm = TRUE)), x = df$Efficiency[sleep])
  # WASO (h:min)
  if ("WASO" %in% colnames(df)) {
    tib[6, 1:3] <- sapply(funs, function(fun, x) hm(fun(x, na.rm = TRUE)), x = df$WASO[sleep])
  }
  return(tib)
}
...
importFiles <- function(filepath, filename) {
  ...
  for (i in 1:length(filename)) {
    ...
    data <- read.csv(filepath[i], header = FALSE, sep = ",", col.names = paste0("V", seq_len(nCol)), fill = TRUE)
...
sleepData <- function(data, rsn, nse) {
  ...
  for (i in ints) {
    ...
    b <- which(data$stats$`Interval Type` == "REST" & data$stats$`Interval#` == i)
    ...
    if ("Onset Latency" %in% colnames(data$stats)) {
      slf[i, 4] <- sum(data$stats$`Onset Latency`[r], na.rm = TRUE) / 60 # hours
      sld[i, 6] <- sum(data$stats$`Onset Latency`[r], na.rm = TRUE) # minutes
    }
    # Sleep duration
    slf[i, 5] <- sum(data$stats$`Sleep Time`[r], na.rm = TRUE) / 60 # hours
    sld[i, 5] <- sum(data$stats$`Sleep Time`[r], na.rm = TRUE) # minutes
    # Time in bed (minutes)
    sld[i, 4] <- sum(data$stats$Duration[b], na.rm = TRUE)
    # Time awake / light sleep (hours)
    slf[i, 6] <- sum(data$stats$`Wake Time`[r], na.rm = TRUE) / 60
    # Sleep efficiency (%)
    slf[i, 8] <- sld[i, 7] <- sum(data$stats$Efficiency[r], na.rm = TRUE)
    # Wake after sleep onset (minutes)
    if ("WASO" %in% colnames(data$stats)) {
      sld[i, 8] <- sum(data$stats$`WASO`[r], na.rm = TRUE)
    }
    # Number of wake bouts (#)
    if ("#Wake Bouts" %in% colnames(data$stats)) {
      sld[i, 9] <- sum(data$stats$`#Wake Bouts`[r], na.rm = TRUE)
    }
  }
  rm(i, r)
...
# divide data into menstrual cycles and phases
menstCycles <- function(data, C1, C2, C1P2, C1P3, C2P2, C2P3) {
  tib <- cbind(timeInBed(data$stats, C1[1], C1[2]), timeInBed(data$stats, C2[1], C2[2]),
               timeInBed(data$stats, C1[1], C1P2-1), timeInBed(data$stats, C1P2, C1P3-1), timeInBed(data$stats, C1P3, C1[2]),
               timeInBed(data$stats, C2[1], C2P2-1), timeInBed(data$stats, C2P2, C2P3-1), timeInBed(data$stats, C2P3, C2[2]))
  
  # Menstrual cycle data and figures spreadsheets
  sld <- data$sld
  ...
  sld <- cbind(sld[, 1, drop = FALSE], Cycle, Phase, sld[, 2:9, drop = FALSE])
  
  slfC1 <- data$slf[which(data$slf$Date >= C1[1] & data$slf$Date <= C1[2]), ]
  slfC1 <- cyc(slfC1, C1P2, C1P3)
  slfC2 <- data$slf[which(data$slf$Date >= C2[1] & data$slf$Date <= C2[2]), ]
  slfC2 <- cyc(slfC2, C2P2, C2P3)
  
  # Mean, min, max bed and wake times
  bt <- cbind(bwt(data$sld, C1[1], C1[2]), bwt(data$sld, C2[1], C2[2]), 
              bwt(data$sld, C1[1], C1P2-1), bwt(data$sld, C1P2, C1P3 - 1), bwt(data$sld, C1P3, C1[2]),
              bwt(data$sld, C2[1], C2P2-1), bwt(data$sld, C2P2, C2P3 - 1), bwt(data$sld, C2P3, C2[2]))
  
  data$sld <- sld
  data$slfC1 <- slfC1
  data$slfC2 <- slfC2
  data$dex <- cbind(data$dex, rbind(bt, tib))
  return(data)
}
...
sleepReport <- function(data, srtemp) {
  ...
  cells <- getCells(rows)
  setCellValue(cells$`8.4`, gsub("_", " ", data$subject))
  setCellValue(cells$`9.4`, Sys.Date())
  
  # Add summary table
  addDataFrame(data$dex, sheets$Summary, startRow = 3, startColumn = 2, col.names = FALSE, row.names = FALSE)
  
  # Add data for figures
  addDataFrame(data$slf, sheets$Figures, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, 
               showNA = TRUE)
  addDataFrame(data$slfC1, sheets$`Figures C1`, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, 
               showNA = TRUE)
  addDataFrame(data$slfC2, sheets$`Figures C2`, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, 
               showNA = TRUE)
  
  # Add data for analysis
  addDataFrame(data$sld, sheets$Data, startRow = 2, startColumn = 1, col.names = FALSE, row.names = FALSE, 
               showNA = TRUE)
...
durationPlot <- function(slf, rsn) {
  ...
  ggplot(df) +
    ...
    geom_hline(aes(yintercept = rsn, linetype = "Recommended sleep per night (h)"), color = "#BE4B48", size = 1.25) +
    theme_minimal() +
...
patternPlot <- function(slf, sld) {
  ...
  ggplot(df) +
    ...
    coord_cartesian(ylim = as.POSIXct(c(paste(Sys.Date() - 1, "19:00:00"), paste(Sys.Date(), "11:00:00")), tz = "GMT"),
                    xlim = c(min(df$Date) - 0.75, max(df$Date) + 0.75), expand = 0) +
    scale_x_date(date_breaks = "2 days", date_labels = "%d/%m") +
    geom_hline(yintercept = mbt, color = "#BE4B48", size = 1.25) +
    geom_hline(yintercept = mwt, color = "#BE4B48", size = 1.25) +
    theme_minimal() +
